...
class Client:
    def __init__(self, server_ip, server_port, p2p_port, hostname=None):
    ...
    def _start_p2p_listener(self):
        ...
        try:
            ...
            p2p_socket.listen(5)
            logging.debug(f"P2P listener started on port {self.p2p_port}")
    ...
    def _do_publish(self, lname, fname, allow_overwrite=False):
        ...
        target_ext = os.path.splitext(fname)[1]
        if source_ext and source_ext != target_ext:
            fname = os.path.splitext(fname)[0] + source_ext
        publish_message = {
            'action': 'publish',
            'lname': lname,
            'fname': fname,
    ...
    def run(self):
        ...
        logging.debug(f"Client starting with P2P port: {self.p2p_port}")
        ...
        try:
            logging.debug(f"Connecting to server at IP: {self.server_ip} - Port:{self.server_port}...")
...
def _run_cli_client(p2p_port, hostname=None):
    client_instance = Client(server_ip='10.156.12.139', server_port=9999, p2p_port=p2p_port, hostname=hostname)
    client_instance.run()

if __name__ == "__main__":
    if not (2 <= len(sys.argv) <= 3):
        print("Usage: python client.py <my_p2p_port> [client_name]")
        sys.exit(1)
    ...
    client_name = sys.argv[2] if len(sys.argv) == 3 else None

    sys.modules.setdefault("client", sys.modules[__name__])
    try:
        from client_ui import main as client_ui_main
    except Exception as exc:
        logging.error(f"Unable to launch client UI: {exc}")
        logging.info("Falling back to CLI mode.")
        _run_cli_client(p2p_port, client_name)
    else:
        client_ui_main(
